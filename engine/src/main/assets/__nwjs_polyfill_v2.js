...
(function () {
    ...
    // ---- JsonEx 根修复：替换为 v1 (community-1.3b / 1.6.x) 逻辑 ----
    // 游戏自带 rpg_core.js 的 JsonEx（1.3.x 及更老）在序列化稀疏数组时不保留
    // Array 原型标记，读档后退化为 plain object（{0:1, 2:3}），导致
    // map/filter/forEach 等方法丢失 → 此前所有 "is not a function" 崩溃的根因。
    // v1 的 JsonEx 用 @a 包装器保数组实例，读档后仍是真实 Array。
    // 对 1.6.x 游戏（自带同款 @a 包装）本替换为幂等无操作。
    (function () {
        try {
            if (typeof window.JsonEx === "undefined") return;
            JsonEx.maxDepth = 100;
            JsonEx._id = 1;
            JsonEx._generateId = function () { return JsonEx._id++; };
            ...
            JsonEx._restoreCircularReference = function (circulars) {
                circulars.forEach(function (circular) {
                    var key = circular[0]; var value = circular[1]; var content = circular[2];
                    value[key] = content;
                });
            };
            ...
            JsonEx._linkCircularReference = function (contents, circulars, registry) {
                circulars.forEach(function (circular) {
                    var key = circular[0]; var value = circular[1]; var id = circular[2];
                            ...
                            if (value[key] && typeof value[key] === "object") {
                                if (value[key]["@c"]) {
                                    ...
                                    if (value[key] instanceof Array) {
                                        circular.push([key, value, value[key]]);
                                        value[key] = { "@c": value[key]["@c"], "@a": value[key] };
                                    }
                                }
                            } else {
                                value[key] = this._encode(value[key], circular, depth + 1);
            ...
            console.log("[v2] JsonEx replaced with v1 (community-1.3b) logic");
        } catch (eJsonEx) {
            console.warn("[v2] JsonEx replacement failed", eJsonEx && eJsonEx.message);
        }
    })();
